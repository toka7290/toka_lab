...
void fill_white_canvas(Image image){
  int i,j;
...
Color **make_data(int width, int heght){
    int i;
    Color **data;
...
void draw_line(Image image, int x1, int y1, int x2, int y2, Color linecolor){
  int i;
  int x = x1,y = y1;
...
void draw_rect(Image image, int x1, int y1, int x2, int y2, Color linecolor){
...
void draw_quad(Image image, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, Color linecolor){
...
void draw_circle(Image image, int cx, int cy,int extent, Color linecolor){
  double r;
...
void draw_sinewave(Image image, int x1, int y1, int x2, int y2, int extent,double freq,Color linecolor){
  int dx = abs(x2 - x1);
  int dy = abs(y2 - y1);
  int sx = (x1 < x2) ? 1 : -1;
  int sy = (y1 < y2) ? 1 : -1;
...
void fill_bucket_s(Image image,int x,int y, Color color, Color targetcolor){
...
void fill_bucket(Image image,int x,int y, Color color){
...
void replace_color(Image image, Color replace_col, Color color){
  int i,j;
  for(i=0;i<image.width;i++){
    for(j=0;j<image.heght;j++){
